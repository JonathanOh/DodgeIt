...
//        layer.borderColor = UIColor.getRGBFromArray(currentPlayer.randomMapTheme.backgroundColor)?.cgColor//CONSTANTS.COLORS.BACKGROUND_VIEW.cgColor//UIColor.black.cgColor
        ...
        if isSafe() {
            backgroundColor = .clear//UIColor.getRGBFromArray(currentPlayer.randomMapTheme.courseColor)//CONSTANTS.COLORS.PUZZLE_CONTAINER_VIEW
            layer.borderWidth = 0
        }
...
//            let obstacleImageView = UIImageView(image: obstacleImage)
            let obstacleImageView = currentPlayer.randomMapThemeNew.getObstacleTileImageView()
...
//            let grassImageView = UIImageView(image: grassCollection[Int.randomUpTo(2)])
            let grassImageView = currentPlayer.randomMapThemeNew.getCourseTileImageView()
...
//        explosionAnimationImageView.animationDuration = 0.4
        let explosionAnimationImageView = currentPlayer.randomMapThemeNew.getExplosionAnimatedImageView()
